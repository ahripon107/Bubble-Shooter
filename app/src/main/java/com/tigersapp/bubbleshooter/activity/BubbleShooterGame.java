...
public class BubbleShooterGame extends PuzzleGameScreen {
    ...
    public boolean play(boolean key_left, boolean key_right, boolean key_fire, double trackball_dx, double[] touch_dx) {
        ...
        if (this.endOfGame) {
            ...
            long shootNumber = GameConfig.getInstance().get(BubbleShooterActivity.SHOOT_NUMBER_OF_SKIP_CHANCE_KEY, 0) + 1;
        ...
        if (this.movingBubble != null) {
            ...
            if (this.movingBubble.fixed()) {
                if (this.movingBubble.getSpritePosition().y >= 380 && !this.movingBubble.released()) {
                    ...
                    this.fixedBubbles++;
                    this.blinkDelay = 0;
                    if (this.fixedBubbles >= this.levelManager.getMaxFixedBubbleCount()) {
                        this.fixedBubbles = 0;
                        sendBubblesDown();
                    }
                }
